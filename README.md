...
从 [Release 页面](https://gitcode.com/weixin_41523975/classnode/releases) 下载源码压缩包（如 `classnode-v1.4.8x.x.zip`），解压。
...
>
> **💡 提示：学生机连不上？** 通常是防火墙拦截了 3001 端口，在教师电脑上任选一种方式：<br>
> **方式一（放行软件）**：打开「Windows 安全中心 → 防火墙和网络保护 → 允许应用通过防火墙」，勾选 **ClassNode** 或 **Node.js** 的专用和公用网络。<br>
> **方式二（开放端口）**：在「高级设置 → 入站规则 → 新建规则」，依次选：端口 → TCP → 特定本地端口 3001 → 允许连接。
