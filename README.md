...
<details>
<summary>v1.3.6 — 2026-06-07（当前版本）</summary>

#### 桌面端状态面板窗口
- 新增桌面窗口，实时显示服务状态、访问地址、局域网 IP
- 支持窗口内一键启动/停止服务
- 地址一键复制与打开浏览器
- 系统托盘左键显示面板，右键弹出菜单

#### 核心优化
- AI 请求新增 30 秒超时控制，避免上游 API 挂起时连接泄漏
- 课堂创建改为批量写入，50 名学生从 100 次 DB 操作降至 2 次
- 屏蔽词加入内存缓存，避免每次消息都查询数据库
- 数据库增加索引，提升查询性能

#### 修复
- 修复 AI 请求无超时的问题
- 修复内存泄漏（Anonymizer 500 条上限自动清理）
- 修复数据库备份为 0KB 的问题
- 修复屏蔽词禁用后仍生效的问题
- 修复学生输入框含英文时不清除的 bug
- 修复打包脚本和端口注释错误
