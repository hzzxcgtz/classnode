...
const sections = [
  ...
  },
  {
    ...
    items: [
      ...
      <><strong>导入学生</strong>（支持三种方式）：</>,
      <div style={{ paddingLeft: 20, lineHeight: 2, fontSize: 14, color: '#475569' }}>
        <div>• <strong>手动添加</strong>：逐个输入姓名、学号、标签（选填），适合少量学生录入。</div>
        <div>• <strong>粘贴名单</strong>：直接粘贴学生姓名列表（每行一个姓名），系统自动生成序号作为学号，适合快速建班。</div>
        <div>• <strong>批量导入</strong>：下载模板后，上传填写好的 JSON 或 CSV 文件进行批量导入，适合大规模数据迁移。</div>
      </div>,
      <>学生列表内支持随时编辑姓名、修改标签或删除个别学生，灵活调整班级成员。</>,
  ...
  },
  {
    ...
    items: [
      ...
      <><strong>身份认证</strong>：学生输入互动码，根据课堂模式选择自己的「姓名」或「小组」，即可无缝加入课堂。</>,
      <><strong>开始互动</strong>：进入界面后，学生即可看到 AI 智能体头像和对话窗口。支持发送文字消息、上传图片附件，所有对话记录将实时同步至教师端，实现真正的双向可视化教学。</>,
    ],
  },
  {
    title: '六、课堂看板监控（核心功能）',
    ...
    items: [
      ...
      <><strong>深度查阅与投屏</strong>：点击任意学生卡片，右侧抽屉将展示该生的完整对话记录（支持一键清除记录重置会话）。点击卡片右上角的「全屏」按钮，可将特定对话投屏展示，全屏模式下顶部工具栏支持调节卡片列数（2-6 列），方便大屏多维对比。</>,
      <><strong>课堂控制</strong>：顶部工具栏支持一键「暂停课堂」（冻结学生端输入）或「结束课堂」（回收互动码并生成历史档案）。</>,
    ],
  },
  {
    title: '七、历史数据沉淀与导出',
    icon: 'history',
    items: [
      <>课堂结束后，进入 <Highlight>历史数据</Highlight> 页面，这些数据将成为教师宝贵的教研资产。</>,
  ...
  },
  {
    ...
    icon: 'shield',
    items: [
      <>系统提供完善的本地数据保障机制，位于 <Highlight>历史数据</Highlight> 页面底部。</>,
